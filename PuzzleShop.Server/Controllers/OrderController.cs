...
namespace PuzzleShop.Server.Controllers
{
    ...
    {
        private readonly OrdersDAO _ordersDao = new OrdersDAO();
        ...
        public bool SaveOrder()
        {
            ...
            try
            {
                ...
                if (result) HttpContext.Session.Remove("CART"); // delete cart

                return result;
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[ERROR] Save order error {ex.Message}");
                return false;
            }
        }
