...
namespace PuzzleShop.Client.Pages.ToyDetail.Logic
{
    public class ToyDetailLogic : BlazorComponent
    {
        ...
        [Parameter] protected List<Toy> ToyList { get; set; }

        protected Toy CurrentToy { get; set; }

        protected bool AddSuccess { get; set; }
        ...
        protected override void OnParametersSet()
        {
            ...
            if (CurrentToy != null)
            {
                ...
                if (CurrentToy.Review == null) CurrentToy.Review = new Review[0];
            }
        ...
        protected override void OnAfterRender()
        {
            if (CurrentToy != null)
            {
                ...
                var largeImageUrl = "";
                for (var i = 0; i < splittedUrl.Length - 1; ++i) largeImageUrl += splittedUrl[i] + "/";

                largeImageUrl += imageName;
                JSRuntime.Current.InvokeAsync<bool>("zoomImage", "#toy-img-zoom", largeImageUrl);
        ...
        {
            if (CurrentToy.Quantity + offsetValue <= 0) return;
        ...
        {
            if (CurrentToy?.Review == null || CurrentToy.Review.Length == 0) return 0;

            var totalRating = 0;
            foreach (var review in CurrentToy.Review) totalRating += review.Star;
