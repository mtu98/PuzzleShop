...
LsCardForm = function(opts, form)
{
  ...
  // Event handler for frame.validate()
  function validateCvv(inputProperties) {
    if (!inputProperties['validCvv']) {
      highlightFormError({
        attribute: 'cvv',
        key: 'errors.invalid',
        message: 'CVV entered is not valid'
        });
    }
    ...
    if (inputProperties['validNumber'] && inputProperties['validCvv']) {
      LSHandleAjaxData(form, undefined);
    }
  }

  this.frame.on('validation', validateCvv);
...
$(document).on('submit', '[data-ajax-handler]', function(ev) {
  ...
  if (handler === 'shop:onAddCustomerCard' || handler  === 'subscriptions:onAddCard') {
    lsCardFormFrames[this.id].validate();

  } else if (handler === 'shop:onPay') {
